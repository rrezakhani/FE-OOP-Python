...
class element_PF:
        ...
        def update_element_stiffness(self, C, phi_elem):   
            self.K_elem = np.zeros((self.neN*self.dim, self.neN*self.dim))
            w_qp, qp = self.get_w_qp()
        ...
        def compute_B_J_matrices(self, xi, eta): 
            dNdxi = self.get_shape_function_derivative(xi, eta)
        ...
        def compute_B_PF_matrices(self, xi, eta):   
            dNdxi = self.get_shape_function_derivative(xi, eta)
        ...
        def get_shape_function_derivative(self, xi, eta):
            # Derivate of quadrilateral element shape functions                   
        ...
        def compute_element_PF_residual(self, phi_elem, mat):
            ...
            for p in range(self.nqp):            
                ...
                N = self.get_shape_function(xi, eta)
                N = N[np.newaxis]
                B_PF = self.compute_B_PF_matrices(xi, eta)
                ...
                #print("shape of res_phi_elem_qp =", res_phi_elem_qp.shape)
